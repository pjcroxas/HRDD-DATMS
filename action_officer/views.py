...
from travelauth.models import TravelRequest_tbl, Pending_training_report
...
def action_officer(request, pk):
	...
	if pk == 1:
		...
		query['fcount'] =  TravelRequest_tbl.objects.filter(followup=1).count()
		...
		query['newsubmissions'] =  Requestor.objects.filter(status__status__exact="New").count()
		query['fcount'] =  Requestor.objects.filter(followup=1).count()
		query['requestors'] = Requestor.objects.exclude(status__id=3).order_by('-id')
		query['urlpath'] = "scholarl"
		page = request.GET.get('page', 1)
		paginator = Paginator(Requestor.objects.all().order_by('-id'), 100)
...
def new_sub(request, pk):
	...
	if pk == 1: #travel if#
		query['urlpath'] = "travel"
		page = request.GET.get('page', 1)
		paginator = Paginator(TravelRequest_tbl.objects.filter(status__status__exact="New").order_by('-travelreq_id'), 10)
		...
		page = request.GET.get('page', 1)
		paginator = Paginator(Requestor.objects.filter(status__status__exact="New").order_by('-id'), 100)
...
def take(request, tag=0, cat = "", date = "", ln="", fn="", mi= "", pt = "", frm = "", to = "", ds = "", tid = "", last = "", email=""):
	...
	obj1.taketimestamp = datetime.now()
	obj1.save()
	args = {}
	args['last'] = last
	args['tag'] = tag
	...
	his = History_tbl()
	his.a_officer = request.user.first_name+' '+request.user.last_name
...
def takescholar(request, tag=""):
	pending = Status.objects.get(status__exact="For Evaluation")
	obj = get_object_or_404(Requestor, id = tag)
	obj.assigned = request.user
	obj.status = pending
	obj.taketimestamp = datetime.now()
	obj.save()
	his = History_tbl()
	his.a_officer = request.user.first_name+' '+request.user.last_name
	his.scholar_id = tag
	his.status = "For Evaluation"
	his.save()
	subject = 'SUBMISSION'
	recepient = obj.e_mail
	ctx = {
	       'emailquery': obj,
		'statusform': "Take",
		   }
	html_message = render_to_string('action_officer/email_template/scholaremailtemplate.html', ctx)
...
def updatedetails(request, pk):
	...
	if request.method == "POST":
		...
		if form.is_valid():
			...
			recepient = update.e_mail
			sform.lacking_documents = sform.lacking_documents.split(',')
			if sform.status == Status.objects.get(id = 20) or sform.status == Status.objects.get(id = 23) or sform.status == Status.objects.get(id = 21) or sform.status == Status.objects.get(id = 27):
			   subject = sform.status
			   ctx = {
        				'emailquery': update,
					'statusform':sform.status,
    		   }
			   html_message = render_to_string('action_officer/email_template/scholaremailtemplate.html', ctx)
			   plain_message = strip_tags(html_message)
			   send_mail(subject, plain_message, EMAIL_HOST_USER, [recepient], html_message=html_message,  fail_silently = False)
			   if sform.status != Status.objects.get(id=13):
				   sform.ta_signed_upload = None
			history = History_tbl(status = sform.status, a_officer = request.user.get_full_name() , scholar_id = pk)
			history.save()
	...
	if request.method == "POST":
		...
		if form.is_valid():
			...
			if sform.status ==  approved:
			   ...
			   html_message = render_to_string('action_officer/email_template/emailtemplate.html', ctx)
			   plain_message = strip_tags(html_message)
			   print(html_message)
			   send_mail(subject, plain_message, EMAIL_HOST_USER, [recepient], html_message=html_message,  fail_silently = False)
...
def assigned(request, pk):
	query = {
		'mar': Requestor.objects.filter(assigned = request.user).order_by('-taketimestamp'),
		'user': User.objects.filter(username = request.user),
		'designation': Author.objects.filter(user = request.user),
		'user_role' : Author.objects.filter(Q(user = request.user)&Q(role = 3)),
		'user_role_req' : Author.objects.filter(Q(user = request.user)&Q(role = 1)),
	...
	if pk == 1:
		query['urlpath'] = "travel"
		page = request.GET.get('page', 1)
		paginator = Paginator(TravelRequest_tbl.objects.filter(assigned = request.user).order_by('-taketimestamp'), 10)
...
def pendingtrainingreport(request):
	query = {
		'user': User.objects.filter(username = request.user),
		'designation': Author.objects.filter(user = request.user),
		'ptr': Pending_training_report.objects.all(),
	}
	return render(request, 'action_officer/Database/pendingtrainingreport.html', query)

def save_pendingtrainingreport_form(request, form, template_name):
    data = dict()
    if request.method == 'POST':
        if form.is_valid():
            form.save()
...
def updatependingtrainingreport(request, pk):
	user2 = Pending_training_report.objects.get(report_id = pk)
	if request.method == 'POST':
		form = PendingTrainingReportUpdateForm(request.POST, instance=user2)
	else:
		form = PendingTrainingReportUpdateForm(instance=user2)
	return save_pendingtrainingreport_form(request, form, 'action_officer/Database/pendingtrainingreportupdateform.html')
...
def travel_request_stat(request):
	...
	travel_request_per_office = TravelRequest_tbl.objects.filter(Q(agency='DOTr-CO') | Q(agency='DOTr')).values('office')\
...
def followup(request):
	query = {
		...
		'user_role_req' : Author.objects.filter(Q(user = request.user)&Q(role = 1)),
		'user_role_act' : Author.objects.filter(Q(user = request.user)&Q(role = 2)),
		'followed': TravelRequest_tbl.objects.filter(followup=1).order_by('-travelreq_id'),
		'fcount': TravelRequest_tbl.objects.filter(followup=1).count()
		# 'followed' : Requestor.objects.filter(status__status__exact="New")
	}
	return render(request, 'action_officer/followup.html', query)

def followup_sch(request):
	query = {
		'user': User.objects.filter(username = request.user),
		'designation': Author.objects.filter(user = request.user),
		'user_role' : Author.objects.filter(Q(user = request.user)&Q(role = 3)),
		'user_role_req' : Author.objects.filter(Q(user = request.user)&Q(role = 1)),
		'user_role_act' : Author.objects.filter(Q(user = request.user)&Q(role = 2)),
		'followed': Requestor.objects.filter(followup=1).order_by('-id'),
		# 'followed' : Requestor.objects.filter(status__status__exact="New")
	}
	return render(request, 'action_officer/followup_sch.html', query)
