...
class UpdateForm(forms.ModelForm):
    PIE_CHOICES = (
        ("1", "HRDD will register"),
        ("2", "Nominee will register"),
    )

    class Meta:
        model = Requestor
        fields = ('status', 'lacking_documents', 'ta_signed_upload')
    pie = forms.ChoiceField(choices = PIE_CHOICES, required=False, label='Choices:', )
    lacking_documents = forms.CharField(label='Lacking Documents',
                    widget=forms.TextInput(attrs={'placeholder': 'Ex. Format: Signed of copy nomination, Signed copy of REAP'}), required=False)


    def __init__(self,  *args, **kwargs):
        super(UpdateForm, self).__init__(*args, **kwargs)
        self.fields['status'].queryset = Status.objects.filter(Q(status = 'Approved') | Q(status = 'RELEASE FROM HRDD') | Q(status=  'UNABLE TO PROCEED WITH THE APPLICATION DUE TO PENDING SUBMISSION') | Q(status = 'Cancelled') | Q(status = 'New') | Q(status = 'For Evaluation') | Q(status = 'Evaluation: Failed') | Q(status = 'Passed Initial Evaluation') | Q(status = 'Evaluation: LACK DOCUMENTARY REQUIREMENTS') | Q(status = 'Scholarship Acceptance')).order_by('status')
...
class PendingTrainingReportUpdateForm(forms.ModelForm):

    class Meta:
        model = Pending_training_report
        fields = ('first_name','last_name','middle_initial','gender','position','office','training_institution','course_name','duration_in_days',
        'programdates_from','programdates_to','program_fee','post_report_submit_date','echo_seminar_date','remarks')
