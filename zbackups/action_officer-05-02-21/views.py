...
def new_sub(request, pk):
	...
	if pk == 1: #travel if#
		...
		page = request.GET.get('page', 1)
		paginator = Paginator(Requestor.objects.filter(status__status__exact="New"), 100)
...
def updatedetails(request, pk):
	...
	if request.method == "POST":
		...
		if form.is_valid():
			...
			if sform.status == Status.objects.get(id = 20): #lacking_documents
			   history = History_tbl(status = sform.status, a_officer = request.user.get_full_name() , scholar_id = pk)
			   history.save()
			...
			if sform.status == Status.objects.get(id = 23): #unable to proceed
			   history = History_tbl(status = sform.status, a_officer = request.user.get_full_name() , scholar_id = pk)
			   history.save()
			...
			if sform.status == Status.objects.get(id = 21): #evaluation:failed
			   history = History_tbl(status = sform.status, a_officer = request.user.get_full_name() , scholar_id = pk)
			   history.save()
