...
def updatedetails(request, pk):
	...
	if request.method == "POST":
		...
		if form.is_valid():
			...
			if sform.status == Status.objects.get(id=13): #approved
			...
			elif sform.status == Status.objects.get(id = 20): #lacking_documents
			   history = History_tbl(status = sform.status, a_officer = request.user.get_full_name() , scholar_id = pk)
			   history.save()
			   subject = 'LACKING DOCUMENTS'
			   ctx = {
        				'emailquery': update,
						'statusform':sform.status,
    		   }
			...
			elif sform.status == Status.objects.get(id = 23): #unable to proceed
			   history = History_tbl(status = sform.status, a_officer = request.user.get_full_name() , scholar_id = pk)
			   history.save()
			...
			elif sform.status == Status.objects.get(id = 21): #evaluation:failed
			   history = History_tbl(status = sform.status, a_officer = request.user.get_full_name() , scholar_id = pk)
			   history.save()
